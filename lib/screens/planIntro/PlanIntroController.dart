...
/// Controller for the plan intro screen animations and navigation.
class PlanIntroController extends GetxController with GetTickerProviderStateMixin {
  ...
  void _initializeAnimations() {
    ...
    lottieAnimation = Tween<double>(begin: 0.0, end: 1.0).animate(CurvedAnimation(
      parent: mainController,
      curve: const Interval(0.0, 0.6, curve: Curves.elasticOut),
    ));

    headingAnimation = Tween<double>(begin: 50.0, end: 0.0).animate(CurvedAnimation(
      parent: mainController,
      curve: const Interval(0.2, 0.7, curve: Curves.easeOutCubic),
    ));

    subtitleAnimation = Tween<double>(begin: 50.0, end: 0.0).animate(CurvedAnimation(
      parent: mainController,
      curve: const Interval(0.4, 0.9, curve: Curves.easeOutCubic),
    ));

    buttonAnimation = Tween<double>(begin: 0.0, end: 1.0).animate(CurvedAnimation(
      parent: mainController,
      curve: const Interval(0.6, 1.0, curve: Curves.easeOutBack),
    ));

    buttonPressAnimation = Tween<double>(begin: 1.0, end: 0.95).animate(CurvedAnimation(
